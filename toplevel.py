...
class Toplevel(Account):
    ...
    def retrieve_password_toplevel(self):
        pass_var = self.fetch_password(self.web_var, self.user_var)
        ...
        label_frame = tk.Frame(retrieve_pass, width=(TL_WIDTH - 20), height=(TL_HEIGHT * .75))
        label_frame.pack(expand=True, fill=tk.BOTH)
        btn_frame = tk.Frame(retrieve_pass, width=(TL_WIDTH - 20), height=(TL_HEIGHT * .2))
        btn_frame.pack(expand=True, fill=tk.BOTH)
        header = tk.Label(label_frame, text="The password for:", font=FONT_TOPLEVEL)
        header.pack(expand=True, fill=tk.BOTH)
        ...
        user.pack(expand=True, fill=tk.BOTH)
        password = tk.Label(label_frame, text=f"is: {pass_var}", anchor=tk.CENTER, font=FONT_TOPLEVEL)
        password.pack(expand=True, fill=tk.BOTH)
        copy_btn = tk.Button(btn_frame, text="Copy Password & Close", bg=BUTTON_GRAY, command=lambda: [pass_var.get(),
                             retrieve_pass.destroy()])
        copy_btn.pack(expand=True, fill=tk.BOTH)
    ...
    def update_password_toplevel(self):
        ...
        label_frame = tk.Frame(update_pass, width=(TL_WIDTH - 20), height=(TL_HEIGHT * .75))
        label_frame.pack(expand=True, fill=tk.BOTH)
        btn_frame = tk.Frame(update_pass, width=(TL_WIDTH - 20), height=(TL_HEIGHT * .2))
        btn_frame.pack(expand=True, fill=tk.BOTH)
